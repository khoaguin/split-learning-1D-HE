...
class EcgClient256(nn.Module):
    ...
    def __init__(self, context: Context, init_weight_path: Union[str, Path]):
        ...
        self.conv1 = nn.Conv1d(
            in_channels=1, out_channels=16, kernel_size=7, padding=3, stride=1
        )  # 128 x 16
        self.relu1 = nn.LeakyReLU()
        self.pool1 = nn.MaxPool1d(2)  # 64 x 16
        self.conv2 = nn.Conv1d(
            in_channels=16, out_channels=8, kernel_size=5, padding=2
        )  # 64 x 8
...
class Client:
    ...
    def load_ecg_dataset(
        self, train_name: str, test_name: str, batch_size: int
    ) -> None:
        """[summary]
    ...
    def send_context(self) -> None:
        ...
        _ = send_msg(
            sock=self.socket, msg=self.context.serialize(save_secret_key=False)
        )
    ...
    def build_model(self, init_weight_path: Union[str, Path]) -> None:
        ...
        self.ecg_model = EcgClient256(
            context=self.context, init_weight_path=init_weight_path
        )
    ...
    def train(self, hyperparams: dict) -> None:
        ...
        for e in range(epoch):
            print(f"---- Epoch {e+1} ----")
            start = time.time()
            e_train_loss, e_correct, e_samples = self.training_loop(
                verbose, loss_func, optimizer, batch_encrypted
            )
            end = time.time()
    ...
    def training_loop(self, verbose, loss_func, optimizer, batch_encrypted):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            he_a2: CKKSTensor = CKKSTensor.load(context=self.context, data=he_a2_bytes)
            if verbose:
            ...
            server_grads = {"dJda2": dJda2.to("cpu"), "dJdW": dJdW.detach().to("cpu")}
            if verbose:
                print("\U0001F601 Sending dJda2, dJdW to the server")
            _ = send_msg(sock=self.socket, msg=pickle.dumps(server_grads))
            dJda_bytes, _ = recv_msg(sock=self.socket)
            dJda: CKKSTensor = CKKSTensor.load(context=self.context, data=dJda_bytes)
            if verbose:
                print("\U0001F601 Received dJda from the server")
            ...
            if verbose:
                print(f"training time for 1 batch: {end-start:.2f}s")
            # if i == total_batch-1:
            #     break
...
def main():
    ...
    if hyperparams["verbose"]:
        ...
        print(hyperparams)
    client.load_ecg_dataset(
        train_name=project_path / "data/train_ecg.hdf5",
    ...
    #                             [60, 40, 40, 60],
    #                             pow(2, 40))
    client.make_tenseal_context(8192, [40, 21, 21, 40], pow(2, 21))  # 8192a
    ...
    client.send_context()
    client.build_model("weights/init_weight.pth")
    ...
    if hyperparams["save_model"]:
        df.to_csv("./outputs/loss_and_acc_2048_batch.csv")
        torch.save(
            client.ecg_model.state_dict(), "./weights/trained_client_2048_batch.pth"
        )
